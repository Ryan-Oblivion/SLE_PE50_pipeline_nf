...
path out_f_ctr, emit: fastp_file1
path out_r_ctr, emit: fastp_file2
...
path "${out_f_ctr}.bam", emit: coor_sorted_bam
...
# not using picard because its not compatible with bwa aln
# only with bwa mem
...
samtools index -b "${out_f_ctr}.bam" 

#cp "${out_f_ctr}.bam"'*' /scratch/rj931/tf_sle_project/store_bam_files
...
coor_sort_picard(align_bwa.out.bam_file, out_f_ctr)
